...
function printSch() {
    $.ajax({
        ...
        success: function (response) {
            ...
            let contentCategory = []
            //항목별 보기, 난이도와 컨텐츠 조합해서 모든 경우의 row 만들어주기
            for (let i = 0; i < all_sch.length; i++) {
                if (contentCategory.includes('[' + all_sch[i]['difficulty'] + ']' + all_sch[i]['content'])) {
                } else {
                    contentCategory.push('[' + all_sch[i]['difficulty'] + ']' + all_sch[i]['content'])
            ...
            for (let i = 0; i < all_sch.length; i++) {
                ...
                switch (all_sch[i]['day']) {
                    case '월':
                        $('.table_schedule_column_monday > .table_schedule_contents').append(temp_html);
                        break;
                    case '화':
                        $('.table_schedule_column_tuesday > .table_schedule_contents').append(temp_html);
                        break;
                    case '수':
                        $('.table_schedule_column_wednesday > .table_schedule_contents').append(temp_html);
                        break;
                    case '목':
                        $('.table_schedule_column_thursday > .table_schedule_contents').append(temp_html);
                        break;
                    case '금':
                        $('.table_schedule_column_friday > .table_schedule_contents').append(temp_html);
                        break;
                    case '토':
                        $('.table_schedule_column_saturday > .table_schedule_contents').append(temp_html);
                        break;
                    case '일':
                        $('.table_schedule_column_sunday > .table_schedule_contents').append(temp_html);
                        break;
                }

                //항목별, 각 항목에 맞춰 넣어주기
                let categoryScheduleContents = $(`.category_schedule_row[data-difficulty='${all_sch[i]["difficulty"]}'][data-content='${all_sch[i]["content"]}']`)
                categoryScheduleContents.find('.category_schedule_contents').append(temp_html)

                //본인이 참여한 일정의 색을 바꿔주기
                if (schMember.map(row=>row.nickname).includes(tempNickname)) {
                    $('.table_schedule_content[data-id = ' + sch_id_list[i] + '] > .table_schedule_content_title').css('background-color', 'var(--color_dark)')
                }

                //메모의 내용이 있는 일정에 아이콘 표시
                if (all_sch[i]['memo'] != '') {
                    $('.table_schedule_content[data-id = ' + sch_id_list[i] + '] .bi-chat-dots').css('display', 'flex')
                }

                //고정팟이 체크된 일정에 아이콘 표시
                if (all_sch[i]['fixedparty'] == 'true') {
                    $('.table_schedule_content[data-id = ' + sch_id_list[i] + '] .bi-pin-angle').css('display', 'flex')
                }
            }
        }
    })
...
//일정 추가 창에서 추가하기 버튼 클릭시, 일정추가하기 기능
function addSch(){
    ...
    if (memo.length > 300) {
        Swal.fire({
            title: '일정 추가 실패',
            text: '메모의 최대 글자수를 초과했습니다. (최대 300자)',
            icon: 'error',
            confirmButtonColor: '#5b7d97',
        })
        return 0;
    ...
    let repeatsch = $('#fixedparty').is(':checked');
    $.ajax({
            type: 'POST',
            url: '/nextweeksch/api_addsch/',
            dataType: 'json',
            data: {
...
function schDetail(e){
    ...
    $('.detail_sch_item_difficulty > .detail_sch_item_content').text(sch.attr('data-difficulty'))
    $('.detail_sch_item_contentname > .detail_sch_item_content').text(sch.attr('data-content'))
    $('.detail_sch_item_gate > .detail_sch_item_content').text(sch.attr('data-gate'))
    $('.detail_sch_item_day > .detail_sch_item_content').text(sch.attr('data-day'))
    $('.detail_sch_item_time > .detail_sch_item_content').text(sch.attr('data-time'))
    $('.detail_sch_item_memo > .detail_sch_item_content').text(sch.attr('data-memo'))
    ...
    $.ajax({
        type: 'POST',
        url: '/nextweeksch/api_printdetailedschmember/',
        dataType: 'json',
        ...
        success: function (response) {
            if (response['result'] == 'SUCCESS') {
                ...
                } else if (sch['content'] == '쿠크세이튼' || sch['content'] == '카양겔') {
                ...
                if (myMembership == '관리자') {
                    ...
                    let btnDeleteMySch = $('.detail_sch_member_content:nth-child(1)').filter(function () {
                        return $(this).text() === myNickname;
                    })
                    let btnLeaveSchHtml = `<div onclick="detailSchLeaveSch(event)" class="btn_detail_sch_delete_member">X</div>`
                    btnDeleteMySch.parent('.detail_sch_member_row_content').append(btnLeaveSchHtml);
                }
            } else {
                Swal.fire({
                    title: response['title'],
...
function detailSchJoinSch(e) {
    ...
    $('.join_sch_row:nth-child(2) > .join_sch_item:first-child').text(user_nickname);
...
function detailSchLeaveSch(e){
    let sch_id = $('.detail_sch_container').attr('data-id')
    let myNickname = $(e.target).prevAll('.detail_sch_member_content')[1].innerText
    let myClass = $(e.target).prevAll('.detail_sch_member_content')[0].innerText

    $.ajax({
        type: 'POST',
        url: '/nextweeksch/api_leavesch/',
        dataType: 'json',
        data: {
            'sch_id_give': sch_id,
...
function toggleView(){
    ...
    if (toggleViewBtn.className=='btn_toggleView bi bi-filter-square'){
        ...
        toggleViewBtn.classList.replace('bi-filter-square','bi-filter-square-fill')
    }
    //클래스네임 확인해서, 토글
    else{
        tableView.style.display='flex'
        categoryView.style.display='none'
        toggleViewBtn.classList.replace('bi-filter-square-fill','bi-filter-square')
    }
}