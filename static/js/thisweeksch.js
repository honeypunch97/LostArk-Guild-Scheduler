        ...
        success: function (response) {
            ...
            for (let i = 0; i < all_sch.length; i++) {
                ...
                if(all_sch[i]['new'] == 'true'){
                    $('.table_schedule_content[data-id = ' + sch_id_list[i] + '] > .table_schedule_content_title').css('background-color', '#ff7669')
                }

                //본인이 참여한 일정의 색을 바꿔주기
                if (schMember.map(row=>row.nickname).includes(tempNickname)) {
                    $('.table_schedule_content[data-id = ' + sch_id_list[i] + '] > .table_schedule_content_title').css('background-color', 'var(--color_deep_dark)')
                }
